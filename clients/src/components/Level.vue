  ...
  <div>
          ...
          <q-btn flat icon="add" @click="icon = true">
            <q-dialog v-model="icon">
              <q-card class="bg-white text-black add-fact">
                ...
                <q-card-section>
                  <div class="text-h6">Добавление нового уровня сложности</div>
                </q-card-section>
          ...
          <q-tr :props="props">
            ...
            <q-td key="color" :props="props">
              <q-chip :style="{'background-color': `${props.row.color}`}" style="font-size: 16px;" text-color="white">{{ props.row.name }}
                <q-popup-edit v-model="props.row.fact_text" @hide="changeLevelDiffColor(props.row._id, props.row.color)">
    ...
    <q-card>
      ...
      <q-card-actions align="between">
        ...
        <q-btn flat label="Удалить" @click="deleteLevel(deleteRowId)" color="primary" v-close-popup></q-btn>
      </q-card-actions>
    </q-card>
  </q-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    async changeTagName(id, name){
      try {
        const res = await api.put("api/level/"+id, {
          name: name
        },{
          headers: {
            Authorization: "Basic " + btoa(this.auth),
    ...
    async addLevel(){
      try {
        const res = await api.post("api/level", {
          name: this.newName,
          color: this.newColor
        },{
          headers: {
